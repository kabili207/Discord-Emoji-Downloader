...
$(document).ready(function () {
  ...
  $("#default-1 #continue").click(async (e) => {
    ...
    $("#server-select").dropdown({
      ...
      onChange: async (value, text, $selected) => {
        ...
        globalThis.emojis = renameEmoji(globalThis.guild.emojis).sort(
          sortAlpha,
        );
        globalThis.stickers = globalThis.guild.stickers.sort(sortAlpha);

        let emojis = globalThis.emojis.reduce(
          (acc, val, i) => {
            if (i > 149) {
              acc[1].push(val);
            } else {
  ...
  $("#default-2 #submit").click(async (e) => {
    ...
    if (!globalThis.emojis.length)
      return error("Please select at least one emoji.");
    try {
      if (globalThis.guild.emojis.length < 1)
        return error("This server doesn't have any emojis!");
  ...
  $("#manual #submit").click(async (e) => {
    ...
    try {
      ...
      if (!guild.id)
        return error(
          "Your code seems off... are you sure you pasted the guild object?",
        );
